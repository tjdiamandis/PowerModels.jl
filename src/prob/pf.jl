export run_pf, run_pf_tr, run_opf_tr, run_ac_pf, run_dc_pf

""
function run_ac_pf(file, optimizer; kwargs...)
    return run_pf_tr(file, ACPPowerModel, optimizer; kwargs...)
end

""
function run_dc_pf(file, optimizer; kwargs...)
    return run_pf_tp(file, DCPPowerModel, optimizer; kwargs...)
end

""
function run_pf_tr(file, model_constructor, optimizer; kwargs...)
    return run_generic_model(file, model_constructor, optimizer, post_pf_tr; kwargs...)
end
...
""
function post_pf_tr(pm::GenericPowerModel)
...
""
function run_pf_tp(file, model_constructor, optimizer; kwargs...)
    return run_generic_model(file, model_constructor, optimizer, post_pf_tp; kwargs...)
end

""
function post_pf_tp(pm::GenericPowerModel)
    variable_voltage(pm, bounded = false)
    variable_generation(pm, bounded = false)
    variable_branch_flow(pm, bounded = false)
    variable_dcline_flow(pm, bounded = false)
